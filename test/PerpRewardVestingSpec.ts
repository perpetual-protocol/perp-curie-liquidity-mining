...
describe("liquidity mining", () => {
    ...
    describe("claimWeeks()", () => {
        ...
        it("admin claims alice's two shares", async () => {
            ...
            await perpLiquidityMining.connect(admin).claimWeeks(alice.address, claimsArr)
            expect(await token.balanceOf(alice.address)).to.eq(parseEther("500000"))
            expect(await perpLiquidityMining.claimed("2", alice.address)).to.eq(true)
            expect(await perpLiquidityMining.claimed("7", alice.address)).to.eq(true)
        })
        ...
        it("alice & bob both claim their three shares", async () => {
            await perpLiquidityMining.seedAllocations(2, RANDOM_BYTES32_1, parseEther("200000"))
            await perpLiquidityMining.seedAllocations(5, RANDOM_BYTES32_2, parseEther("300000"))
            await perpLiquidityMining.seedAllocations(7, RANDOM_BYTES32_3, parseEther("100000"))
            const claimsArr = [
                {
                    week: "2",
            ...
            expect(await token.balanceOf(perpLiquidityMining.address)).to.eq(parseEther("100000"))
        })
        ...
        it("alice & bob both claim two of their three shares", async () => {
            ...
            await perpLiquidityMining.connect(alice).claimWeeks(alice.address, aliceClaimsArr)
            await perpLiquidityMining.connect(bob).claimWeeks(bob.address, bobClaimsArr)
            expect(await token.balanceOf(alice.address)).to.eq(parseEther("250000"))
            expect(await token.balanceOf(bob.address)).to.eq(parseEther("200000"))
            expect(await token.balanceOf(perpLiquidityMining.address)).to.eq(parseEther("150000"))
        ...
        it("force error, claimWeeks() twice", async () => {
            ...
            await perpLiquidityMining.connect(alice).claimWeeks(alice.address, claimsArr)
            await expect(perpLiquidityMining.connect(alice).claimWeeks(alice.address, claimsArr)).to.revertedWith(
                "Claimed already",
            )
        })
        ...
        it("force error, claiming twice, first claimWeeks() then claimWeek()", async () => {
            ...
            await expect(
                perpLiquidityMining.claimWeek(alice.address, 2, parseEther("100000"), [RANDOM_BYTES32_1]),
            ).to.revertedWith("Claimed already")
        })
    })
})
